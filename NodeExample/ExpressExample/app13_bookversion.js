...
app.use(multer({
    ...
    onFileUploadsStart: function (file)  {
        console.log('File upload starts.');
    },
    onFileUploadComplete: (file, req, res) => {
        console.log('File upload completed :' + file.fieldname + ' -> ' + File.path);
    },
    onFileSizeLimit: (file) => {
        console.log('File size limit exceeded : %s', file.originalname);
    }
}).array('photo'));
...
app.post('/process/photo', (req,res) => {
